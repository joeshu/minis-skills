...
当运行环境不是 Claude Code / 完整 skills runtime，而是 Open Minis 这类会话式环境时，按以下兼容规则执行：
...
则**跳过长篇方法讲解和多轮教育**，直接进入可交付产物模式：
- 能一次输出完整 `SKILL.md` → 直接输出全文
- 一次输出过长 → 主动分段输出，并提示用户回复「继续」
- 不能直接建正式目录 → 先生成到 `/var/minis/workspace/`，不要卡住

### 2. 目录兼容策略
Open Minis 下优先使用：
- 正式目录：`/var/minis/skills/[person-name]-perspective/`
- 临时目录：`/var/minis/workspace/[person-name]-perspective/`

默认策略：
- 用户只是试产物、要先审稿 → 先写 `workspace`
- 用户明确要正式技能 → 直接写 `skills`

### 3. 降级执行规则
如果当前环境不适合真正启动多个 subagent / 自动脚本链路，则按降级流程执行：
1. 用单线程分阶段完成 Phase 1 → Phase 3
2. 保留关键检查点，但可压缩展示
3. 产物优先，自动化其次
4. 不因缺少 runtime 就停留在“解释该怎么做”而不生成结果

### 4. 输出纪律
在 Open Minis 中，优先遵守以下规则：
- 用户要结果，就先给结果
- 用户要文件，就必须落地文件
- 用户要 `SKILL.md`，就不要只给框架或说明
- 如果不能一次吐完，主动分段，不要中途失联

### 5. 最低可交付标准
即使环境受限，也至少要产出以下之一：
- 完整 `SKILL.md`
- 已落地到 `/var/minis/workspace/.../SKILL.md` 的可审稿版本
- 若必须分段，则连续输出直到正文结束，不得只给前言

---

## Open Minis 专用触发模板

在 Open Minis 中，优先使用下面这些明确触发语，避免进入长解释模式。

### 1. 直接生成完整 Skill
适用于：用户已经明确目标，希望直接拿到完整 `SKILL.md`

推荐触发语：
- `造一个「XX Skill」，请直接输出完整 SKILL.md 全文。`
- `蒸馏一个「XX」，不要解释，直接给最终可用版 SKILL.md。`
- `基于我给的材料，直接生成 XX Skill 的完整 SKILL.md。`
...
### 2. 先调研再生成
适用于：用户还没确定方向，或希望先看调研质量

推荐触发语：
- `蒸馏一个 XX，先做调研摘要给我确认。`
- `帮我造一个 XX Skill，先给我提炼出的核心心智模型。`
- `基于这些材料蒸馏 XX，先给我调研检查点。`

系统响应要求：
- 允许走 Phase 1.5 / 2.5 检查点
- 先给调研或提炼摘要
- 用户确认后再构建完整 `SKILL.md`
...
### 5. 当前场景推荐模板
若用户要在 Open Minis 中直接造 Skill，优先推荐用户这样说：
...
### 6. 最小执行闭环（Open Minis 强制规则）
在 Open Minis 中，如果用户已经明确要求“造一个 skill / 直接输出 SKILL.md / 不要解释”，必须优先执行以下最小闭环：

1. **先确定产物名**
   - 若用户已明确名称，直接使用
   - 若未明确，但主题清晰，用最小必要推断命名

2. **先创建落地目录**
   - 默认先建 `/var/minis/workspace/[skill-name]/`
   - 若用户明确要正式技能，再建 `/var/minis/skills/[skill-name]/`
...
4. **再决定是否继续补研究文件**
   - 有余力再补 `references/research/*.md`
   - 没余力时，至少保证 `SKILL.md` 已落地

5. **最后向用户汇报产物路径并继续输出正文**
   - 先告诉用户文件已生成
   - 再在对话里输出全文或分段输出

### 7. 禁止停留在说明层
出现以下任一情形时，视为执行失败：
- 只展示女娲自身的 `SKILL.md` 文档
- 只解释方法论，不产出用户目标技能
- 只给框架、提纲、建议，不落地文件
- 说“已生成”但没有真实路径

遇到用户强制要结果时，宁可先给一个可审稿版本，也不要卡在解释模式。
...
3. **用途**：思维顾问？决策参考？角色扮演？
4. **新建 or 更新**：是否已有该人物的Skill？（检查 `/var/minis/skills/` 目录，必要时也检查 `/var/minis/workspace/` 下的候选skill目录）
5. **本地语料**：「你手上有没有这个人的一手素材？比如书籍PDF、演讲/访谈transcript、视频字幕、个人博客导出等。有的话直接丢给我，比网上搜的质量高得多。」

**Open Minis 快捷规则**：如果用户已经明确要求“直接输出完整 SKILL.md / 不要解释 / 直接造 skill”，则把上述确认压缩到最少：
- 能默认的直接默认
- 有明显缺失信息再追问1次
- 否则直接推进到 Phase 0.5 和后续构建
...
**来源A：本地已有Skill**
扫描 `/var/minis/skills/` 目录下的子目录，读取每个 `SKILL.md` 的 description，匹配用户需求。已有Skill可以即插即用，不需要重新蒸馏。如果扫描结果为空（用户还没有任何可复用skill），跳过此步，只从来源B推荐。
...
**Open Minis 最低要求**：即使后续还要继续调研，此阶段也必须至少创建技能目录并准备 `SKILL.md` 占位文件，避免整个流程只停留在对话里。

```
/var/minis/skills/[person-name]-perspective/
├── SKILL.md                          # 最终产物
...
> Open Minis 兼容约定：如果当前只是先生成会话内可审阅的技能草稿，也可以先落到 `/var/minis/workspace/[person-name]-perspective/`，确认后再整理进 `/var/minis/skills/`。优先保证产物真实落地，不要只停留在对话里。
...
Phase 1启动前，**主动扫描 `/var/minis/skills/` 目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用，比WebSearch更稳定高效：
...
#### Step 4: 输出
将完成的SKILL.md写入 `/var/minis/skills/[person-name]-perspective/SKILL.md`。

> Open Minis 兼容补充：如果当前会话不适合直接创建正式技能目录，至少先写入 `/var/minis/workspace/[person-name]-perspective/SKILL.md`，并把调研文件同步落地。严禁只在对话里声称“已生成”却没有文件产物。

#### Step 4.5: 分段输出规则（Open Minis）
如果用户明确要求“直接输出完整 SKILL.md”，则：
1. 优先先落地文件
2. 再在对话里输出全文，或按章节分段输出
3. 如果长度过长，主动在段末提示“回复继续获取后续正文”
4. 不得因为篇幅长而退回到摘要或说明模式
...
### Open Minis 更新补充
- 如果当前环境中已有正式目录，就直接更新 `/var/minis/skills/.../SKILL.md`
- 如果只是会话内修改，先更新 `/var/minis/workspace/.../SKILL.md`
- 更新任务同样要真实落地文件，不得只在对话中口头说明“已更新”
