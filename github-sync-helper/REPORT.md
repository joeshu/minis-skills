...
说明：当前环境未启用独立子 agent 自动双跑，因此本报告采用 **with-skill vs baseline 的人工对照法**。本轮重点验证：复杂 Git 场景、风险确认、分支/PR 选择、只内容替换、API 操作、列表输出、冲突边界，以及脚本入口与文档一致性。
...
### Case 9：脚本入口与文档一致性
**Prompt**：按文档里的统一脚本入口执行常见 GitHub 同步命令。
...
- 高风险、复杂冲突、API 管理、协作流和脚本入口一致性都已形成更完整闭环
- 技能从“方法强”进一步提升为“文档与实现一致、可直接使用”
