...
## 失败回退策略
### 开始前阶段失败
- 未命中专题记忆：回退到 `GLOBAL.md` 与 daily memory
- 专题存在但不足：先补查 `GLOBAL.md`，再按需补查 daily memory

### 执行中阶段失败
- 无法判断新规则属于哪层记忆：默认先不升格长期规则，优先写 daily memory
- 同主题零散长期记忆很多，但无法安全归并：先记录归并建议，不强行整理
...
### 跨会话继续模板
- 开始前：查专题 + 最近变化
- 结束后：摘要默认写入 `/var/minis/shared/`
- 删除历史前：先确认 handoff 可读

### 记忆治理联动模板
- 开始前：查该主题长期规则
- 执行中：若出现稳定新规则，判断是否写专题或 GLOBAL
- 结束后：必要时再调用 `open-minis-memory-store`
